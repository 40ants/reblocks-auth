(uiop:define-package #:reblocks-auth/providers/email/processing
  (:use #:cl)
  (:import-from #:reblocks-auth/providers/email/models
                #:registration-email
                #:check-registration-code)
  (:import-from #:reblocks/html
                #:with-html)
  (:import-from #:dexador)
  (:import-from #:log)
  (:import-from #:reblocks/actions
                #:*js-default-form-action*)
  (:import-from #:reblocks/widget
                #:defwidget)
  (:import-from #:reblocks-auth/button)
  (:import-from #:reblocks-ui/form
                #:with-html-form
                #:render-form-and-button)
  (:import-from #:reblocks-ui/popup
                #:hide-popup
                #:show-popup)
  (:import-from #:reblocks-lass)
  (:import-from #:reblocks/dependencies)
  (:import-from #:reblocks-auth/core
                #:*allow-new-accounts-creation*)
  (:import-from #:reblocks-auth/models
                #:get-user-by-email)
  (:import-from #:serapeum
                #:fmt)
  (:import-from #:reblocks-auth/auth)
  (:import-from #:reblocks/request)
  (:import-from #:yason)
  (:import-from #:secret-values
                #:ensure-value-revealed)
  (:import-from #:log4cl-extras/secrets
                #:with-secrets)
  (:export #:*recaptcha-site-key*
           #:*recaptcha-secret-key*
           #:*smartcaptcha-client-key*
           #:*smartcaptcha-server-key*
           #:request-code-form
           #:render-submit-button
           #:render-email-input
           #:form-css-classes
           #:render-sent-message
           #:retpath
           #:sent))
(in-package #:reblocks-auth/providers/email/processing)


(defvar *recaptcha-site-key* nil
  "Set this variable to a site key, generated by Google reCaptcha.")

(defvar *recaptcha-secret-key* nil
  "Set this variable to a secret key, generated by Google reCaptcha.")

(defvar *smartcaptcha-client-key* nil
  "Set this variable to a client key, generated by Yandex SmartCaptcha.")

(defvar *smartcaptcha-server-key* nil
  "Set this variable to a server key, generated by Yandex SmartCaptcha.")

(defvar *smartcaptcha-api-url* "https://smartcaptcha.yandexcloud.net/validate"
  "API endpoint for verifying Yandex SmartCaptcha tokens.")


(defwidget request-code-form ()
  ((retpath :initarg :retpath
            :reader retpath)
   (sent :initform nil
         :accessor sent)))


(defwidget request-code-form-for-popup (request-code-form)
  ())


(defwidget request-code-popup (reblocks-ui/popup:popup-widget)
  ((form :initarg :form
         :type request-code-form-for-popup
         :reader form)))


(defun verify-recaptcha (token)
  "More about token verification:

   https://developers.google.com/recaptcha/docs/verify
"
  (log:debug "Verifying recaptcha token")
  
  (unless *recaptcha-secret-key*
    (error "To verify recaptcha requests you need to set reblocks-auth/providers/email/processing::*recaptcha-secret-key*"))
  
  (let ((payload (list
                  (cons "secret" *recaptcha-secret-key*)
                  (cons "response" token)))
        (ip (reblocks/request:get-remote-ip)))
    (when ip
      (push (cons "remoteip" ip)
            payload))
    (let* ((response
             (dex:post "https://www.google.com/recaptcha/api/siteverify"
                       :content payload
                       :connect-timeout 3
                       :read-timeout 3))
           (parsed (yason:parse response))
           (success (gethash "success" parsed)))
      (if success
          ;; Here is an example of successful response:
          ;; {
          ;;   "success": true,
          ;;   "challenge_ts": "2023-10-21T21:39:04Z",
          ;;   "hostname": "localhost",
          ;;   "score": 0.9,
          ;;   "action": "submit"
          ;; }
          (log:debug "Received recaptcha response" response)
          (log:error "Received recaptcha response" response))
      (values success)))) 


(defun verify-smartcaptcha (token)
  "Verifies SmartCaptcha token via Yandex API.
   
   SmartCaptcha sends POST request with token and secret key.
   Returns T on success, NIL on failure."
  (with-secrets (token)
    (log:debug "Verifying smartcaptcha token")
  
    (unless *smartcaptcha-server-key*
      (error "To verify smartcaptcha requests you need to set *smartcaptcha-server-key*"))

    (unless token
      (error "Smart captcha widget did not send us a smart-token parameter."))
    
    (let* ((payload (list (cons "secret" (ensure-value-revealed *smartcaptcha-server-key*))
                          (cons "token" token)))
           (ip (reblocks/request:get-remote-ip)))
      (when ip
        (push (cons "ip" ip) payload))
    
      (let* ((response (dex:post *smartcaptcha-api-url*
                         :content payload
                         :connect-timeout 3
                         :read-timeout 3))
             (parsed (yason:parse response)))
        (log:debug "SmartCaptcha response" response)

        (let ((status (gethash "status" parsed)))
          (string-equal status "ok"))))))


(defmethod reblocks/widget:render ((widget request-code-form))
  (flet ((send-code (&key email captcha-token smart-token &allow-other-keys)
           (cond
             ((string= email "")
              (reblocks-ui/form:field-error "email"
                                            "Please, give a correct email address."))
             
             ((and *smartcaptcha-server-key*
                   (not (verify-smartcaptcha smart-token)))
              (reblocks-ui/form:form-error "Captcha verification failed. Please try again."))
             
             ((and *recaptcha-secret-key*
                   (not *recaptcha-site-key*)
                   (not (verify-recaptcha captcha-token)))
              (reblocks-ui/form:form-error "Seems you are a robot. Please, contact with site's owner."))
             
             ((and (not *allow-new-accounts-creation*)
                   (null (get-user-by-email email)))
              (reblocks-ui/form:field-error "email"
                                            (fmt "Учётная запись с email \"~A\" не найдена."
                                                 email)))
             
             (t
              (setf (sent widget)
                    t)
              (reblocks-auth/providers/email/models::send-code email :retpath (retpath widget))
              (reblocks/widget:update widget)))))
    (cond
      ((sent widget)
       (render-sent-message widget))
      (t
       (let ((smartcaptcha-p (when *smartcaptcha-client-key*
                               t))
             (recaptcha-p (when (and *recaptcha-site-key*
                                     (not *smartcaptcha-server-key*))
                            t)))
         (with-html-form (:post #'send-code
                           :class (format nil "~{~A~^ ~}"
                                          (uiop:ensure-list
                                           (form-css-classes widget)))
                           :submit-fn (cond
                                        (smartcaptcha-p
                                         "submitFormWithSmartCaptcha(\"~A\", $(this))")
                                        (recaptcha-p
                                         "submitFormWithRecaptcha(\"~A\", $(this))")
                                        (t
                                         *js-default-form-action*)))
           
           (when smartcaptcha-p
             (:script :src "https://smartcaptcha.yandexcloud.net/captcha.js")
             (:div :style "height: 100px; margin-bottom: 2rem"
                   :id "captcha-container"
                   :class "smart-captcha"
                   :data-sitekey (ensure-value-revealed *smartcaptcha-client-key*)))
           
           (when recaptcha-p
             (:script :src (format nil "https://www.google.com/recaptcha/api.js?render=~A"
                                   *recaptcha-site-key*))
             
             (:script
              (:raw (format nil "
       function submitFormWithRecaptcha(actionCode, form) {
         grecaptcha.ready(function() {
           grecaptcha.execute('~A', {action: 'submit'}).then(function(token) {
               var options = {'args': {'recaptcha-token': token}};
               initiateFormAction(actionCode, form, options);
           });
         });
       }
     "
                            *recaptcha-site-key*))))
           
           (render-email-input widget)
           (reblocks-ui/form:error-placeholder "email")
           (render-submit-button widget)))))))



(defgeneric render-sent-message (widget)
  (:method ((widget request-code-form))
    (with-html ()
      (:p "A login link was sent to your email.")))
  
  (:method ((widget request-code-form-for-popup))
    (call-next-method)
    
    (flet ((close-popup (&rest rest)
             (declare (ignore rest))
             (setf (sent widget)
                   nil)
             (hide-popup widget)))
      (render-form-and-button :close #'close-popup))))


(defgeneric render-email-input (widget)
  (:method ((widget request-code-form))
    (with-html ()
      (:input :name "email"
              :type "email"
              :placeholder "Email"))))


(defgeneric render-submit-button (widget)
  (:method ((widget request-code-form))
    (with-html ()
      (:input :type "submit"
              :class "button small"
              :value "Send Code"))))


(defgeneric form-css-classes (widget)
  (:method ((widget request-code-form))
    nil))


(defmethod reblocks-ui/popup:render-popup-content ((widget request-code-popup))
  (reblocks/widget:render (form widget)))


(defmethod reblocks-auth/button:render ((service (eql :email))
                                        &key retpath)
  (let ((popup (make-instance 'request-code-popup
                              :form (make-instance 'request-code-form-for-popup
                                                   :retpath retpath))))
    (with-html ()
      (reblocks/widget:render popup)

      (render-form-and-button
       :email (lambda (&rest args)
                (declare (ignore args))
                (show-popup popup))
       :button-class "button small"))))


(define-condition new-accounts-are-prohibited (error)
  ())


(defmethod reblocks-auth/auth:authenticate ((service (eql :email)) &rest params &key code)
  (declare (ignorable params))
  
  (unless code
    (error "Unable to authenticate user without the code."))
  
  (let* ((registration-code (check-registration-code code)))
    (let* ((email (registration-email registration-code))
           (user (reblocks-auth/models:find-social-user :email email)))
      
      (cond
        (user (values user
                      nil))
        (t (values (reblocks-auth/models:create-social-user :email ;; service
                                                            email  ;; user-id
                                                            ;; metadata
                                                            :email email)
                   t))))))


(defmethod reblocks/dependencies:get-dependencies ((widget request-code-popup))
  (list*
   (reblocks-lass:make-dependency
     '(.request-code-popup
       (.popup-content
        :color black)))
   (call-next-method)))
